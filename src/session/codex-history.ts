...
function listRequest(options: ListCodexThreadHistoryOptions) {
  return {
    ...
    params: {
      ...
      cwd: options.cwd,
      // An omitted modelProviders filter defaults to the currently configured
      // provider in Codex. Include all providers so sessions created before a
      // provider switch remain visible in the resume list.
      modelProviders: [],
